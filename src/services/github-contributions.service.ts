...
export interface GithubContributionsMeta {
  ...
  privateContributionsMode:
    'eligible' | 'limited' | 'disabled' | 'profile_preference';
...
export class GithubContributionsService {
  ...
  private async loadUserContributions(
    username: string,
  ): Promise<GithubContributionsResult> {
    let profileUser: GithubUser | null = null;
    try {
      profileUser = await this.fetchUserFromProfileContributionGraph(username);
    } catch (error) {
    ...
    const githubPat = `${this.configService.get('github.pat') || ''}`.trim();
    if (!githubPat) {
      throw new ServiceUnavailableException(
        'GitHub contribution graph unavailable and GITHUB_PAT is not configured',
      );
    }
    ...
    if (!githubRes.ok || !githubData) {
      throw new ServiceUnavailableException(
        `GitHub GraphQL fallback failed with status ${githubRes.status}`,
      );
    }
    ...
    const privateContributionsReason = {
      ...
      privateContributionsMode
    ] as GithubContributionsMeta['privateContributionsReason'];

    const user = githubData?.data?.user;
    if (!user && githubData?.errors?.length) {
      throw new ServiceUnavailableException('GitHub GraphQL fallback failed');
    }
  ...
  private async fetchUserFromProfileContributionGraph(username: string) {
    ...
    if (pages.some((page) => page.status && page.status !== 200)) {
      return null;
    }
