...
describe('getTheme', () => {
  ...
  it('should expose distinct dark and light palettes for the cool dual-mode themes', () => {
    ...
    expect(getTheme(PresetTheme.LASER_GRID, true)).toEqual([
      '#070A12',
      '#1B2A6B',
      '#00D4FF',
      '#9B5CFF',
    ...
    expect(getTheme(PresetTheme.MATRIX_RAIN, false)).toEqual([
      '#F7FFF8',
      '#D8F9D2',
      '#92E897',
      '#30B95A',
  ...
  it('should format preset labels for display', () => {
    ...
    expect(formatPresetThemeLabel(PresetTheme.OCEAN_REACTOR)).toBe(
      'Ocean Reactor',
    );
  });
  ...
  it('should expose primary tones for every theme definition', () => {
    ...
    expect(themeDefinitions[PresetTheme.PRISM_BREAK].primaryTones).toEqual([
      'red',
      'yellow',
      'cyan',
      'purple',
