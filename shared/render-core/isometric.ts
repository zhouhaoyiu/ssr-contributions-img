...
export const isometricProcessor = (
  ...
  const getShadeColor = (color: string, percent: number) => {
    const key = `${color}:${percent}`;
    const cached = shadeCache.get(key);
    if (cached) return cached;

    const shaded = shadeColor(color, percent);
    shadeCache.set(key, shaded);
    return shaded;
  };

  const shadeColorLeft = (color: string) =>
    getShadeColor(color, -(15 + light));
  const shadeColorRight = (color: string) =>
    getShadeColor(color, -(5 + light));
  const shadeColorTop = (color: string) => getShadeColor(color, -10 + light);
  ...
  const cubeBuilder = ({
    ...
    const stroke =
      strokeWidth > 0
        ...
        : {};
    const attrs = { class: className } as Record<string, number | string>;
