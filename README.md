<div align="center">
  ...
  <p><sub>上图由本仓库部署的 Cloudflare Worker 生成，数据来自 GitHub 用户 <code>zhouhaoyiu</code>。</sub></p>
</div>
...
下面的截图来自当前仓库的 Vue Playground，使用 `zhouhaoyiu` 的真实贡献数据和 `prism_break` 主题。

![Vue Playground 展示 zhouhaoyiu 的真实贡献数据](./assets/screenshots/playground-zhouhaoyiu.png)
...
| 当前在线地址 | `ssr-contributions-img.zhouhaoyiu.workers.dev` | 本地或自行部署 |
...
Cloudflare Worker 入口是 `worker/index.ts`，配置见 `wrangler.toml`。Worker 适合公开 SVG 图片链接，根路径会跳转回本仓库。

Nest/Vercel 入口是 `api/index.ts`，配置见 `vercel.json`。需要 PNG、JPEG、HTML、主题总览或完整 Playground API 时使用这一部署方式。

## 致谢

本项目基于 [CatsJuice/ssr-contributions-img](https://github.com/CatsJuice/ssr-contributions-img) 的开源实现继续维护。3D 柱图的原始实现思路见作者的 [Medium 文章](https://medium.com/@catsjuice/fake-3d-bar-chart-with-svg-js-134684bd5100) 和 [CodePen 示例](https://codepen.io/catsjuice/pen/MWVqNdQ)。
